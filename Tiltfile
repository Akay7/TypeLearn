...
# --- The database, on the host ------------------------------------------------
# The suite that counts runs in the pod ("Run backend tests" below) and in CI,
# inside the image that ships. This forward exists for the editor: VS Code's
# Testing view runs pytest on the host, so with the database reachable there, a
# breakpoint in a test is an ordinary breakpoint — no second debugging
# arrangement to remember, no attach step, and one test can be run on its own.
#
# The credentials are written where the editor can read them rather than left
# for a developer to copy out of a Secret by hand. .tilt/ is git-ignored and
# tiltignored, so the file is neither committed nor watched — and it is outside
# the backend's build context, so writing it neither triggers a rebuild nor ends
# up inside an image.
#
# PGSSLMODE=disable is not incidental, and removing it breaks the suite in a way
# that looks like anything but TLS. CNPG serves TLS; a TLS client that
# disconnects leaves Postgres resetting the connection, and `kubectl
# port-forward` treats a reset as fatal for the WHOLE forward rather than for
# that one connection. With TLS on, exactly one connection ever succeeds:
...
# thing that can fail a run for a reason unrelated to the change under test.
if not CI_PREBUILT:
  local_resource(
    "postgres-forward",
    serve_cmd="""
        for i in $(seq 1 60); do
          kubectl get secret postgres-app -n NAMESPACE >/dev/null 2>&1 && break
          echo "waiting for the database's credentials ($i)..."; sleep 3
        done
        umask 077
        mkdir -p .tilt
        {
          echo "# Generated by Tilt, for the editor's Testing view. Valid only"
          echo "# while postgres-forward is running; nothing in the cluster"
          echo "# reads it, and .tilt/ is git-ignored."
          echo "POSTGRES_HOST=127.0.0.1"
          echo "POSTGRES_PORT=DBPORT"
          echo "POSTGRES_DB=$(kubectl get secret postgres-app -n NAMESPACE -o jsonpath='{.data.dbname}' | base64 -d)"
          echo "POSTGRES_USER=$(kubectl get secret postgres-app -n NAMESPACE -o jsonpath='{.data.username}' | base64 -d)"
          echo "POSTGRES_PASSWORD=$(kubectl get secret postgres-app -n NAMESPACE -o jsonpath='{.data.password}' | base64 -d)"
          echo "PGSSLMODE=disable"
        } > .tilt/backend-test.env
        kubectl port-forward -n NAMESPACE svc/postgres-rw DBPORT:5432
    """.replace("NAMESPACE", NAMESPACE).replace("DBPORT", str(DB_PORT)),
    # Green only once the port actually answers, so a test run started the
    # moment Tilt goes green is not racing the forward.
    readiness_probe=probe(
        period_secs=5,
        tcp_socket=tcp_socket_action(port=DB_PORT, host="127.0.0.1"),
    ),
    resource_deps=["postgres"],
    labels=["data"],
  )
...
# --- Buttons ------------------------------------------------------------------
# A management command runs in the pod, against the environment it really has —
# the forward above is for the editor's test runs and nothing else. These are the
