...
**Reason**: The database was the only containerized piece, which is what made
local development diverge from a deployment: the backend and frontend ran on the
host as two separate origins, and that divergence is the reason the backend
needed CORS at all. The whole stack now runs in a `kind` cluster, where
PostgreSQL is a workload like any other.

**Migration**: `podman-compose down` and remove the `typelearn-db-data` volume
and the `typelearn-net` network, then delete `db.env`. Install `kind`, `tilt`,
`kubectl`, and `helm`, and run `tilt up`. Podman or Docker is still required —
`kind` runs its nodes on one of them — so the project's preference for Podman is
unaffected. Data is not migrated: the exercise catalog is regenerated by
re-running ingestion, which is deterministic and produces the same 100 exercises.
