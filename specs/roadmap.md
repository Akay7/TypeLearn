...
| 2026-08-28 | kind drives Docker, not Podman | kind 0.32 cannot drive podman 6 — its `ps --format` template errors, so `kind get clusters` fails and nothing scripted against kind works. This overrides the 2026-04-21 Podman preference for the cluster only |
